...
namespace MonsterBlaster.Systems
{
    public class PhysicsSystem
    {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            var player = ComponentManager.Get().GetComponents<PlayerComponent>().First();
            var playerPosition = ComponentManager.Get().EntityComponent<PositionComponent>(player.Key);
            ...
            foreach (var movementComponent in movementComponents)
            {
                ...
                float distance = Vector2.Distance(position.Position, playerPosition.Position);

                float turnSpeed = 1f;
                position.Orientation = TurnToFace(position.Position, playerPosition.Position, position.Orientation, turnSpeed);

                Vector2 heading = new Vector2(
                (float)Math.Cos(position.Orientation), (float)Math.Sin(position.Orientation));
                position.Position += heading * speed;
